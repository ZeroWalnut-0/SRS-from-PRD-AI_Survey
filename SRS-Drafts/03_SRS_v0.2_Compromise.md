...
```mermaid
sequenceDiagram
    participant Scheduler as 삭제 스케줄러 (Vercel Cron)
    participant DB as Database
    participant Storage as Supabase Storage
    ...
    loop 각 만료 문서에 대해
        Scheduler->>Storage: 원본 문서 파일 삭제
        Storage-->>Scheduler: 삭제 완료
...
| KPI | 측정 주기 | 측정 경로 | 목표 |
|---|---|---|---|
| 북극성 KPI: 유료 ZIP 다운로드 완료 건수 | 매주(Weekly) | PG 결제 완료 콜백 API 호출 수 + DB `payment_cleared=true` 레코드 → Prisma 쿼리 대시보드 집계 | 월 10,000건 |
| 보조 KPI 1: 폼 파싱 완료율 | 매일(Daily) | Vercel Analytics + AUDIT_LOG `doc_id` 인입 건수 대비 `form_id` 발급 비율 | ≥ 95% |
